...
class MCTS:
    ...
    def initialize_children(self, depth=3):
        self._expand(self.root)
        for i in range(depth):
            old_children = deepcopy(self.children)
            for parent, children in old_children.items():
    ...
    def _select(self):
        ...
        while True:
            ...
            if node not in self.children:
                self._expand(node)
            unexplored = [child for child in self.children[node] if child not in self.children.keys()]
            if random.random() < (1 / self.exploration_weight):
    ...
    def best_child(self, c_param=1.4):
        ...
        for child in self.children:
            if not child.get_n() == 0:
                children_weights.append((child.get_q() / child.get_n()) +
                                        c_param * np.sqrt((2 * np.log(self._n)) / child.get_n()))
            else:
