...
def early_game_heuristic(state : CatanState, player : AbstractPlayer):
    """
    ...
    brick_count = player.get_resource_count(Resource.Brick)
    lumber_count = player.get_resource_count(Resource.Lumber)
    wool_count = player.get_resource_count(Resource.Wool)
    grain_count = player.get_resource_count(Resource.Grain)
    ore_count = player.get_resource_count(Resource.Ore)

    # what is our trading ratio for each resource
    brick_trade_ratio = calc_player_trade_ratio(player, state, Resource.Brick)
    lumber_trade_ratio = calc_player_trade_ratio(player, state, Resource.Lumber)
    wool_trade_ratio = calc_player_trade_ratio(player, state, Resource.Wool)
    grain_trade_ratio = calc_player_trade_ratio(player, state, Resource.Grain)
    ore_trade_ratio = calc_player_trade_ratio(player, state, Resource.Ore)

    # the number of unexposed development cards, except for VP dev cards.
    num_dev_cards = sum(player.unexposed_development_cards) - player.unexposed_development_cards[DevelopmentCard.VictoryPoint]

    avg_vp_diff = get_avg_vp_difference(scores_by_players, player)
    max_vp_diff = get_max_vp_difference(scores_by_players, player)

    can_build_settlement = 1 if player.can_settle_settlement() else 0
    can_build_city = 1 if player.can_settle_city() else 0
    can_build_dev_card = 1 if player.has_resources_for_development_card() else 0
